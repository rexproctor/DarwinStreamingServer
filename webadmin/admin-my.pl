...
$ssl_available = 0;
if (!$@) {#if no error, error information is stored in $@
	$use_ssl = 1;
...
$streamingadminserver_path = $0; #D:\Github\Dss\admin.pl
open(SOURCE, $streamingadminserver_path);
<SOURCE> =~ /^#!(\S+)/; $perl_path = $1;  #extract any white space
close(SOURCE);
@streamingadminserver_argv = @ARGV;

if($^O eq "MSWin32") { #set the default config path
...
#step: read streamingadminserver.conf, put config into hash:  $config
if(!open(CONF, $conf)) {
...
foreach $k (keys %config) { #set $mime type
    if ($k !~ /^addtype_(.*)$/) { next; }
    $mime{$1} = $config{$k};
}
...
#step: todo return: startplaylist.pl
...
#Step: to return #put messages into hash %messagesfile
