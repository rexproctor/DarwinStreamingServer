...
while(1) {
    ...
    # add ssl socket to select mask, only do this if the Net::SSLeay module is available
    if ($ssl_available) {
    ...
    if ($nonsslrequest || $sslrequest) {#YES
		# got new connection
		if($nonsslrequest) {
		...
		if($^O eq "MSWin32") {
			...
			if ($sslrequest && $ssl_available) {#NO
				...
				Net::SSLeay::use_RSAPrivateKey_file( $ssl_con, $config{'keyfile'}, &Net::SSLeay::FILETYPE_PEM);
				Net::SSLeay::use_certificate_file( 	$ssl_con, $config{'crtfile'}, &Net::SSLeay::FILETYPE_PEM);
